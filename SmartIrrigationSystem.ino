...
// WiFi Credentials
const char* ssid = "Shivani";  // Update with your WiFi SSID
const char* password = "1234567@";  // Update with your WiFi Password
...
// ThingSpeak API
const char* thingSpeakApiKey = "UEFBE2B2QFY7K9H9";  // Replace with your ThingSpeak API Key
...
// Thresholds
#define MOISTURE_THRESHOLD 2600  // Adjust based on sensor calibration
#define TEMP_THRESHOLD 30
#define HUMIDITY_THRESHOLD 80
#define RAIN_THRESHOLD 2000  // Adjust based on sensor calibration
...
void loop() {
    ...
    // Read Rain Sensor (Using Only Analog Value)
    int rainAnalog = analogRead(RAIN_ANALOG_PIN);
    isRaining = (rainAnalog < RAIN_THRESHOLD);  // TRUE if below threshold
    ...
    // **Fixing Pump Control Logic**
    if (soilMoistureValue > MOISTURE_THRESHOLD && !isRaining) {
        Serial.println("Soil is DRY and No Rain. Turning Pump ON.");
        digitalWrite(RELAY_PIN, HIGH);  // Activate pump
        client.publish(mqtt_topic_pump, "ON");
    } else {
        Serial.println("Soil is WET or Raining. Turning Pump OFF.");
